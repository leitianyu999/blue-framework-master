...
import com.leitianyu.blue.context.BeanDefinition;
import com.leitianyu.blue.context.ConfigurableApplicationContext;
import com.leitianyu.blue.exception.ErrorResponseException;
import com.leitianyu.blue.exception.NestedRuntimeException;
import com.leitianyu.blue.exception.ServerErrorException;
import com.leitianyu.blue.exception.ServerWebInputException;
import com.leitianyu.blue.io.PropertyResolver;
import com.leitianyu.blue.utils.ClassUtils;
import com.leitianyu.blue.web.utils.JsonUtils;
import com.leitianyu.blue.web.utils.PathUtils;
    ...
    void doResource(String url, HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        try (InputStream input = ctx.getResourceAsStream(url)) {
            if (input == null) {
                ...
                ServletOutputStream output = resp.getOutputStream();
                byte[] buffer = new byte[8192]; // 或其他适当大小的缓冲区
                int bytesRead;
                    ...
                    switch (param.paramType) {
                        case PATH_VARIABLE:
                            try {
                                ...
                                result = convertToType(param.classType, s);
                        ...
                        case REQUEST_BODY:
                            try (BufferedReader reader = request.getReader()) {
                                result = JsonUtils.readJson(reader, param.classType);
                            } catch (IOException e) {
                                throw new ServerWebInputException("Error reading request body.");
                            }
                            break;
                        case REQUEST_PARAM:
