...
public class BlueyuApplication {
    ...
    public void start(String webDir, String baseDir, Class<?> configClass, String... args) throws Exception {
        ...
        final int javaVersion = Integer.parseInt(versionString.split("\\.")[0]);
    ...
    protected Server startTomcat(String webDir, String baseDir, Class<?> configClass, PropertyResolver propertyResolver) throws Exception {
        ...
        ctx.addServletContainerInitializer(new ContextLoaderInitializer(configClass, propertyResolver), new HashSet<>());
        tomcat.start();
    ...
    protected void printBanner() {
        ...
        // 如果 banner 不为空，则打印其各行内容
        if (banner != null) {
            Arrays.stream(banner.split("\\r?\\n")).forEach(System.out::println);
        }
    }
}
